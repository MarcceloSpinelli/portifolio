...
const About: React.FC<AboutProps> = ({ languageToken }) => {
    ...
    <main className={style.container}>
      <div className={style.modules}>
        ...
        <div className={style.textModule}>
          <p className={style.greetings}>{languageToken.getGreetings()}</p>
          <b className={style.name}>{WORLD}</b>
          <p className={style.role}>{languageToken.getRole()}</p>
          <div className={style.iconsSocial}>
            <a
              href="https://www.linkedin.com/in/marccelo-spinelli-24739b2a2?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app"
              target="_blank"
